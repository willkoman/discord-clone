...
export default async function handler(
    ...
    res:NextApiResponseServerIO
        ...
        try {
            ...
            if(!conversationId) return res.status(400).json({message: "Bad request."});
            if(!directMessageId) return res.status(400).json({message: "Bad request."});
            ...
            if(!directMessage) return res.status(404).json({message: "Message not found."});
